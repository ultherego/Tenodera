...
use crate::handlers::{
    containers, disk_usage, file_list, hardware_info, hosts, journal_query, kdump,
    log_files, metrics_stream, network_stats, networking, packages, storage,
    superuser_verify, system_info, systemd_units, terminal_pty, top_processes,
};
...
pub struct Router {
    ...
    /// Sender for outgoing messages (agent → panel).
        ...
        match msg {
            Message::Open { channel, options } => {
                if let Some(handler) = self.handlers.get(&options.payload).cloned() {
                    if handler.is_streaming() {
                        ...
                        // Track handler for this channel (for future data() calls)
            ...
            Message::Data { channel, data } => {
                ...
                if let Some(handler) = handler {
                    handler.data(&channel, &data).await
                } else {
