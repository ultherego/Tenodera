...
export function Containers() {
  ...
  const [pendingAction, setPendingAction] = useState<{ action: string; id?: string; label: string } | null>(null);
  ...
  const doAction = (action: string, id: string, extra: Record<string, unknown> = {}) => {
    setLoading(true);
    setError(null);
    sendAction(action, { id, ...extra });
  };

  const requestPrivileged = (action: string, label: string, id?: string) => {
    if (su.active) {
      /* superuser mode — execute immediately with stored password */
      setLoading(true);
      setError(null);
      const extra: Record<string, unknown> = { password: su.password };
      if (id) extra.id = id;
      sendAction(action, extra);
      return;
    }
    setPendingAction({ action, id, label });
    setPassword('');
    setError(null);
  };
  ...
  const confirmAction = () => {
    ...
    if (pendingAction.id) extra.id = pendingAction.id;
    sendAction(pendingAction.action, extra);
  ...
  const handlePull = () => {
    ...
    setLoading(true);
    setError(null);
    sendAction('pull', { image: pullImage.trim() });
    setPullImage('');
  };
                        ...
                        <div style={S.actions}>
                          ...
                          <button style={{ ...S.actBtn, color: '#f7768e' }} onClick={() => {
                            if (confirm(`Remove container ${getContainerName(c)}?`))
                              doAction('remove', id, { force: state === 'running' });
                          }} title="Remove">✕</button>
                        </div>
                      </td>
                    </tr>
                        ...
                        <td style={S.td}>
                          <button style={{ ...S.actBtn, color: '#f7768e' }} onClick={() => {
                            if (confirm(`Remove image ${getImageName(img)}?`))
                              doAction('remove_image', id, { force: false });
                          }} title="Remove">✕</button>
                        </td>
                      </tr>
                    );
